...
def system_to_ntp_time(timestamp: float) -> float:
    ...
    if ntp_time >= 2 ** 32:
        raise ValueError("Timestamp {} is beyond NTPv3 rollover".format(timestamp))
    return ntp_time
...
def main():
    ...
    parser.add_argument('--ip', type=str, default='0.0.0.0', help='The IP address to listen on. Default is 0.0.0.0')
    parser.add_argument('--port', type=int, default=123, help='The port to listen on. Default is 123.')
    ...
    parser.add_argument('--time', type=float, default=datetime.datetime.now().timestamp(), help='The time to respond with (in epoch diff). Default is "datetime.datetime.now().timestamp()".')
    parser.add_argument('--time-step', type=int, default=0, help='The amount of time to increment the time by each time a request is received. If not set, then the time will increment normally.')
    ...
    server.RequestHandlerClass.args = args # Store the arguments in the server object, thanks Python 
